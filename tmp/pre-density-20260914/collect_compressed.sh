...
for path in sorted(glob.glob('/storage/penghongen/tmp/pocketxmol_density_20260914/benchmarks/*/result.json')):
 ...
 result[path]=data
print(base64.b64encode(gzip.compress(json.dumps(result,ensure_ascii=False).encode())).decode())
PY
