...
六模型均已从规定官方初始参数完成有效训练、原val/loss选择best、完整测试及CPU评价，全部报告与总体比较见[总日志](../../../日志/总日志.md)。中心模型各做C0和C5，包络模型做E，共10个模型／协议组合、30个重叠视图汇总；每协议446实例、每实例50候选和100步，全部223000个候选生成并评价成功。没有运行训练后完整验证集采样的新任务。
