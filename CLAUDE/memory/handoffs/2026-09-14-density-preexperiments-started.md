...
SMILES代理已用371591的8个CPU进程完成初轮66878实例迁移，报告0失败、14.94秒；主代理只读读取报告核实。随后原始模型图全量核查发现28650实例（42.8392%）键类别不等价；仅从SMILES独立Kekulize后仍有16477实例（24.6374%）不能匹配旧输入。主代理已明确问用户是否接受统一公共芳香编码及相应改变验收目标，尚未收到答复。SMILES正式切换暂停，不能把sanitize后的图同构当成GPU等价，也不能按1%例外排除系统差异。371591诊断已自然退出，after_lock及资源保留。

一个非测试样本还显示原生bf16旧输入重复运行自身有11%—12%梯度差异，FP32新旧对照约7.7e-7；诊断中只将消息求和暂升FP32便消除旧重复差异。正式归约与精度均未改变，不能把单样本诊断扩写为全量GPU通过。完整报告位置及数值见预实验总日志。
...
等待用户对SMILES键编码的明确决定，保留候选代码和全部诊断，不静默放宽门限或改正式精度。密度性能独立使用原已验收图链继续，不能依赖未通过的SMILES链。主代理集成后执行两遍自查，独立布局／Git、注释、逻辑各一轮全面审查，再仅复核既有问题；函数布局例外由用户指定的审查代理批准。
