...
必须接受用户已提交的日志重整：每个实验只有一份日志，当前状态、产物、best、W&B及结果在前，失败／中断／并行尝试在后，不恢复旧文件。每次检查得到新进度，更新对应实验与[总日志](../../../日志/总日志.md)的状态快照；阶段结束或明确分叉才写handoff，普通step或单次验证不新建handoff。当前规则见[AGENTS.md](../../../AGENTS.md)及[整理交接](2026-09-13-experiment-log-consolidation.md)。
