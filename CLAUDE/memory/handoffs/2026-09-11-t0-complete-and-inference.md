...
- [T0推理与评价记录](../../../日志/第一类实验（不加密度信息）/04-B-C-T0-RA推理与评价.md)：正式命令、配置、预算、窄核及运行来源。
- [T0修复与重训记录](../../../日志/第一类实验（不加密度信息）/02-T0中心契约修复与重训.md)：纠偏、全部验收和正确训练停止／best证据。
- [T1-RA预登记](../../../日志/第一类实验（不加密度信息）/03-B-C-T1-RA.md)：当前未启动，等待T0推理评价收口。
