...
confidence 相加的两个布尔检查及碰撞计数。本轮排除肽分支；源码保留的 PDB 读取
路径只作为既有通用接口记录。
"""
...
def calc_clash(inputs, th=1):
    ...
    输入参数:
        ...
        - inputs.pred_path: str，预测配体 SDF 路径；本轮小分子必须以 ``.sdf`` 结尾。
