"""
把训练随机数或采样进度映射为分子各自由度的“保留真值信息比例”。

构象生成与小分子 docking 的噪声器直接使用 :class:`MolInfoLevel`：level=1 表示完全保留当前真值，
level=0 表示完全采用先验样本；同一个分子内同一自由度共享一个标量 level，再广播为逐原子、逐半边、
逐刚体域或逐可旋转键向量。本模块不落盘，返回 0..1 区间内的浮点张量。

旧的 ``IndividualInfoLevel``、``PresetInfoLevel``、``WholeInfoLevel`` 与 ``DiffInfoTraj`` 不在当前
``BaseSampleNoiser`` 主路径中，本轮不展开其字段。
"""
import torch
...
def register_info_level(name):
    """把旧式信息等级类登记为 ``INFO_LEVEL_DICT[name]``；同名注册会覆盖先前类。"""
    def decorator(cls):
        # ``INFO_LEVEL_DICT[name]``：str 到未实例化信息等级类的注册项。
...
def get_level(name, *args, **kwargs):
    """实例化 ``INFO_LEVEL_DICT[name]`` 指向的旧式信息等级类；当前 ``MolInfoLevel`` 不经此工厂构造。"""
...
class DiffInfoTraj(nn.Module):
    def __init__(self, config):
        super().__init__()
        # ``self.config``：dict/EasyDict，保存 scale_start/scale_end/width 三个标量。
        self.config = config
