...
class GaussianExplodePrior(nn.Module):
    """
    为每个原子坐标独立添加按 ``1-info_level`` 线性缩放的零均值高斯噪声。

    配置字段:
        - sigma_max: float, 基础高斯标准差，单位 Å；保存为随模块迁移的 buffer。
        - sigma_func: str|None, 可选按分子大小调整标准差；docking free 训练使用 ``sqrt``。

    前向输入:
        - x: (N, 3), 原子局部坐标，单位 Å。
        - info_level: (N,)|(N,3), 每个原子或坐标分量的信息保留比例。
        - from_prior: bool, True 时 level=0 的坐标直接取 noise，不包含 x。
        - mol_size: (N,)|None, 每个原子所属分子的原子数；同一分子内数值相同，与 x 第一维对齐。

    前向输出:
        - pert: (N, 3), ``x + (1-level)*noise``；level=1 精确恢复 x，from_prior 且 level=0 精确等于 noise，单位 Å。

    ``sigma_func``:
        - None: 每个坐标分量标准差均为 sigma_max。
        - sqrt: 第 i 个原子标准差为 ``max(sigma_max*sqrt(mol_size[i]), 1)`` Å。
        - linbias/sqrtbias/seg59: 保留的其他任务实验映射；不在当前构象/docking 配置中使用。
    """
    def __init__(self, config):
        super().__init__()
        # ``config.sigma_max``：float|缺省，逐坐标高斯基础标准差，单位 Å，缺省 1。
        # ``config.sigma_func``：str|None，按逐原子 ``mol_size`` 调整标准差的函数名。
        # ``self.config``：EasyDict，保留上述高斯尺度与分子大小映射叶。
        self.config = config
        # ``sigma_max``：float，基础逐坐标高斯标准差，单位 Å。
        sigma_max = getattr(config, 'sigma_max', 1) 
        # self.sigma_max = nn.Parameter(torch.tensor(sigma_max), requires_grad=False)
        self.register_buffer('sigma_max', torch.tensor(sigma_max))
        # ``self.sigma_func``：str|None，按 mol_size 调整逐原子标准差的函数名。
        self.sigma_func = getattr(config, 'sigma_func', None)
        
    @torch.no_grad()
    def add_noise(self, x, info_level, from_prior, mol_size=None):
        """把逐原子 level 广播到 XYZ，并按配置标准差生成同形高斯坐标扰动。"""

        if info_level.dim() < x.dim():
            # ``info_level_exp``：[N] -> [N,1] -> [N,3]，每个原子的标量 level 广播到 XYZ 三个分量。
            info_level_exp = info_level[:, None].expand_as(x)
        else:
            # ``info_level_exp``：(N,) 或 (N, 3)，输入已与坐标同维数时直接使用。
            info_level_exp = info_level
        ...
        if mol_size is None or self.sigma_func is None:
            # ``noise``：Tensor，形状为 (N, 3)；与 x 同 dtype/device 的高斯噪声容器，随后原地采样 Normal(0, sigma_max)。
        ...
        # ``pert``：Tensor，形状为 (N, 3)；线性而非平方根权重混合；level 越低，附加噪声幅度越大。
        pert = x + (1 - info_level_exp) * noise
        if  from_prior:
            # ``pert``：Tensor，形状为 (N, 3)；level=0 分量直接取 noise，彻底移除原坐标 x。
            pert = torch.where(info_level_exp == 0, noise, pert)
            
        # ``pert``：Tensor，形状为 (N, 3)；level=1 分量精确恢复 x，避免浮点混合误差。
