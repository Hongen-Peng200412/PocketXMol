...
``halfedge_in``、``step`` 和当前预测状态。denovo、maskfill、肽设计等 noiser 不在本轮逐字段范围中。
"""
...
class DenovoSampleNoiser(BaseSampleNoiser):
    ...
    def sample_level(self, step, batch):
        """为通用分子类别、坐标与半边自由度采样对齐的信息等级。

        输入参数:
            - step: float|None，采样期规范进度；训练期为 None 并触发随机等级。
            - batch.node_type: LongTensor，形状为 (N,)，用于取得原子与坐标自由度数量。
            - batch.halfedge_type: LongTensor，形状为 (H,)，用于取得半边自由度数量。

        返回字段:
            - level_dict.node: FloatTensor，形状为 (N,)，逐原子类别信息等级。
            - level_dict.pos: FloatTensor，形状为 (N,)，逐原子坐标信息等级。
            - level_dict.halfedge: FloatTensor，形状为 (H,)，逐半边类别信息等级。
        """
        level_dict = {}
        # ``level_node``：FloatTensor，形状为 (N,)，逐原子类别信息等级。
        # ``level_pos``：FloatTensor，形状为 (N,)，逐原子坐标信息等级。
        # ``level_halfedge``：FloatTensor，形状为 (H,)，逐半边类别信息等级。
        ...
        level_dict.update({
            # ``node``：FloatTensor，形状为 (N,)，逐原子类别信息等级。
            f'node': level_node,
            # ``pos``：FloatTensor，形状为 (N,)，逐原子坐标信息等级。
            f'pos': level_pos,
            # ``halfedge``：FloatTensor，形状为 (H,)，逐半边类别信息等级。
...
class ConfSampleNoiser(BaseSampleNoiser):
    ...
    构造参数 ``config``:
        ...
        - pre_process: None|str, 可选输入硬约束；支持 ``fix_closest`` 与 ``fix_some``。
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        # ``self.pre_process``：None|Literal[fix_closest,fix_some]，在先验之后、fixed 硬恢复之前约束部分输入坐标；本轮标准构象/docking 配置为 None。
        ...
        # ``self.post_process``：None|str|EasyDict，把网络坐标投影或修正成下一步 ``node_pos``；本轮标准构象/docking 配置为 None。
        self.post_process = config.get('post_process', None)
    ...
    def reassign_in_node(self, batch, in_dict):
        ...
        输入字段:
            ...
            - batch.is_atom_remain: 可选；存在时表示截断肽等不适用情形，本函数整体跳过。
            - in_dict.node: int64, (N,), 加噪后的原子类别。
    ...
    def additional_process(self, batch, in_dict):
        """
        在基类 fixed 恢复前，为已知原子位置施加 ``fix_closest`` 或 ``fix_some`` 约束。
        ...
        输出副作用:
            - fix_closest: 缓存 ``node_closest``，把这些原子的 ``fixed_pos`` 置 1，并把 node_pos/in_dict.pos 恢复为 gt_node_pos。
            - fix_some: 对所有 fixed_pos=True 原子，从 gt_node_pos（若存在）或 node_pos 恢复 in_dict.pos。
        """
        ...
        if self.post_process is None:
            ...
            if step > 0.2:  # directly fixed
                if 'orig_fixed_pos' not in batch:
                    # ``self.pre_process``：str；令下一步 additional_process 恢复所有 fixed 原子输入坐标。
                ...
                if 'orig_fixed_pos' in batch:
                    # ``self.pre_process``：None，低 step 阶段停止下一次 add_noise 的 fix_some 输入预处理。
