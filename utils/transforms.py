...
口袋 kNN 图、fixed prompt、刚体域索引、可旋转键注释及可选真值副本。其他分子设计任务的变换类
保留在同一注册表中，但不属于本轮逐字段阅读范围。
"""
...
class FeaturizePocket(object):
    ...
    def __call__(self, data:PocketMolData):
        ...
        if 'is_atom_remain' in data:  # apply cut for pep, so need to update pocket
            ...
            # ``is_atom_remain``：BoolTensor，形状为 (N,)；旧肽截断保留掩码；由于前一行无条件 raise，以下旧代码不可达。
            is_atom_remain = data['is_atom_remain']
            # ``pocket_pos``：FloatTensor，形状为 (P, 3)；不可达旧分支中的口袋坐标，单位 Å。
            pocket_pos = data['pocket_pos']
            # ``node_pos``：FloatTensor，形状为 (N, 3)；不可达旧分支中的首个配体构象，单位 Å。
            node_pos = data['pos_all_confs'][0]
            # ``dist``：FloatTensor，形状为 (P, N)，不可达旧肽裁剪分支的口袋—配体两两距离，单位 Å。
            dist = torch.norm(pocket_pos[:, None] - node_pos[None], dim=-1) # (n_pocket, n_node)
            # ``threshold``：float 标量，不可达旧分支中“每个口袋点最近配体距离”的最大值，单位 Å。
            threshold = dist.min(dim=1)[0].max()
            # ``is_pocket_remain``：BoolTensor，形状为 (P,)；不可达旧分支的口袋原子保留掩码。
            is_pocket_remain = dist[:, is_atom_remain].min(dim=1)[0] < threshold
            # ``data.pocket_atom_feature``：[P, 25] -> [P_keep, 25]；不可达旧分支按保留掩码裁剪口袋特征。
            data['pocket_atom_feature'] = data['pocket_atom_feature'][is_pocket_remain]
            # ``data.pocket_pos``：[P, 3] -> [P_keep, 3]；不可达旧分支同步裁剪口袋坐标，单位 Å。
...
class FeaturizeMol(object):
    ...
    输出样本字段:
        ...
        - is_peptide: int64, (N,), 小分子为全 0，肽为全 1；本轮只消费全 0 分支。
    ...
    def __init__(self, config):
        ...
        self.exclude_keys = [
            ...
            'mmpa',                      # MMPA 分解辅助对象；当前两个任务不消费。
            'brics',                     # BRICS 分解辅助对象；当前两个任务不消费。
            'peptide_pos',               # 肽原子坐标；不属于本轮小分子任务。
            'peptide_atom_name',         # 肽原子名称；不属于本轮小分子任务。
            'peptide_is_backbone',       # 肽主链标记；不属于本轮小分子任务。
            'peptide_res_id',            # 肽残基标识；不属于本轮小分子任务。
            'peptide_atom_to_aa_type',   # 肽原子到氨基酸类别映射；不属于本轮小分子任务。
            'peptide_res_index',         # 肽原子到残基 0-based 索引映射；不属于本轮小分子任务。
            'peptide_seq',               # str，肽序列；不属于本轮小分子任务。
            'peptide_pep_len',           # int，肽长度；不属于本轮小分子任务。
            'peptide_pep_path',          # str，肽结构路径；不属于本轮小分子任务。
        ]
    ...
    def __call__(self, data: Mol3DData):
        ...
        if data.get('task', '') == 'linking':
            # ``idx``：int，固定为 0；linking 选择第一个构象，避免片段索引与随机构象不一致。
            idx = 0
        else:
        ...
        if self.is_peptide:
            # ``data.is_peptide``：LongTensor，形状为 (N,)；肽配置为全 1，本轮小分子不走此分支。
...
class ConfTransform:
    ...
    配置字段:
        ...
        - fix_some: str|dict|None, 可选额外固定原子策略；小分子默认 None，肽残基分支不属于本轮范围。
    ...
    def __init__(self, config, **kwargs):
        ...
        # ``config.fix_some``：str|Mapping|None，可选已知原子固定规则；本轮小分子配置为 None。
        # ``self.config``：EasyDict，保留上述 setting 权重与可选固定规则叶。
        self.config = config
        ...
        # ``self.fix_some``：None|str|dict，可选已知原子坐标固定规则；小分子构象与 docking 默认 None。
    ...
    def __call__(self, data: PocketMolData):
        ...
        输出字段:
            ...
            - data.domain_node_index: LongTensor，形状为 (2, K)，刚体域—原子归属索引。
            - data.tor_bonds_anno: LongTensor，形状为 (T, 3)，扭转执行层级和轴端点。
            - data.twisted_nodes_anno: LongTensor，形状为 (W, 2)，扭转行号和随动原子编号。
            - data.dihedral_pairs_anno: LongTensor，形状为 (Q, 3)，扭转行号和二面角外侧端点。
            - data.gt_node_type: LongTensor，形状为 (N,)，非训练模式的原子类别真值副本。
            - data.gt_node_pos: FloatTensor，形状为 (N, 3)，非训练模式的局部坐标真值副本，单位 Å。
            - data.gt_halfedge_type: LongTensor，形状为 (H,)，非训练模式的半边类别真值副本。
            - data.fixed_halfdist_flex: LongTensor，形状为 (H,)，非训练模式的原始柔性距离约束。
        """
        # sample setting
        ...
        if 'is_atom_remain'  in data:  # peptide is cut. only applicable for free setting
            raise ValueError('not supported anymore: is_atom_remain')
            # ``setting``：str，不可达旧肽截断分支原计划强制使用 ``free``；前一行已无条件抛错。
            setting = 'free' 
        data.update({
    ...
    def set_fixed(self, data, setting):
        """
        为构象/docking 写入固定类别、可变坐标和 setting 对应的固定距离 prompt。
        ...
        额外固定:
            - ``fix_some`` 非 None 时会把选中原子的 ``fixed_pos`` 改为 1；当前小分子构象/docking 配置不启用该分支。
        """
