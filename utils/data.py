...
def edge_index_to_index_of_edge(edge_index, batch_node):
    ...
    输入参数:
        - edge_index: int64, (2, E), 半边端点的批次级原子编号；每列满足 ``edge_index[0] < edge_index[1]``，数值索引拼接后的 ``node_type`` 第一维。
        - batch_node: int64, (N_all,), 每个原子所属的分子编号；取值范围为 ``[0, B - 1]``。

    返回值:
        - id_edge: int64, (E,), 每条半边在所属分子的上三角完全图中的行优先编号；编号从 0 开始，分子之间不共享编号空间。

    关键前提:
        - 同一列的两个原子必须属于同一分子；函数依赖 ``batch_node`` 找到该分子的原子区间。
        - ``batch_node`` 按分子连续排列，符合 ``torch_geometric.data.Batch`` 的默认拼接方式。
    """
    assert (edge_index[0] < edge_index[1]).all()
    # ``num_nodes_batch``：int64, (B,), 每个分子的原子数；第 b 个值同时决定该分子完全图的边编号上限。
    num_nodes_batch = batch_node.bincounts()
    # ``index_mol``：标量分子编号；现有写法由端点编号与各分子原子数的差推断所属分子。
    index_mol = (edge_index - num_nodes_batch).abs().min()[1]
    # ``bias``：标量批次原子偏移；从批次级端点编号中减去后得到分子内原子编号。
    bias = num_nodes_batch[:index_mol]
    # ``edge_index_in_mol``：LongTensor，形状为 (2, E)；每列两个数值索引所属分子的 ``node_type`` 第一维，范围为 ``[0, N_b - 1]``。
    edge_index_in_mol = edge_index - bias
    # ``id_edge_0``：int64，形状为 (E,)，每条半边较小端点的分子内原子索引。
    # ``id_edge_1``：int64，形状为 (E,)，每条半边较大端点的分子内原子索引。
    id_edge_0, id_edge_1 = edge_index_in_mol
    # ``id_edge``：[2, E] -> [E]；按上三角矩阵逐行累计，把端点对 ``(i, j)`` 映射到唯一半边编号。
...
def inc_func_mol3d(self, key, value, *args, **kwargs):
    ...
    if key == 'bond_index':
        ...
        return len(self['node_type'])
    # 下列字段保存分子片段中的原子编号；即使本轮只学习构象与 docking，也必须保持通用批处理契约。
