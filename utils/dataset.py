...
``make_split`` 与 ``RegenDataset`` 不在本轮构象生成和小分子 docking 的训练/正式测试主路径中，
保留原实现供以后沿数据预处理或二次生成流程继续阅读。
"""
...
def make_split(df_dict, provided_test=[], test_size_dict=None,
    ...
    # ``df_split.data_id``：str 列，按 train、val、test 三组顺序连接的样本标识。
    # ``df_split.split``：str 列，与 ``data_id`` 行对齐的 train/val/test 划分标签。
...
class ForeverTaskDataset(IterableDataset):
    ...
    单样本读取:
        ...
        - is_peptide: LongTensor，形状为 (N,)，本轮小分子任务为全 0。
    ...
    def get_data_key(self, task, db_name, data_id):
        ...
        构象/docking 相关分支:
            ...
            - protacdb: ``mols/<data_id>``，没有独立扭转与分解子库。
        ...
        if task == 'linking':
            # ``data_id_split``：list[str]，linking 可把 data_id 编码为 ``id/index_sep``；conf/dock 不进入此分支。
            data_id_split = data_id.split('/')
            if len(data_id_split) == 2:  # actually it is sep_id
                # ``data_id``：str，linking 复合标识中的基础记录标识。
                # ``index_sep``：str，linking 复合标识中的分隔方案编号。
                data_id, index_sep = data_id_split
            else:
                # ``data_id``：str，非标准 linking 标识只取斜杠前首段作为基础 data_id。
                data_id = data_id_split[0]
                # ``index_sep``：str，缺少分隔编号时回退到记录键 ``0``。
                index_sep = '0'
        ...
        if task == 'linking':
            # str，在现有复合键末尾追加 linking 特定片段与分隔编号。
            key += f';linking/{data_id}/{index_sep}'
        elif task == 'growing':
            # str，在现有复合键末尾追加 growing 特定片段。
...
class LMDBDatabase(Dataset):
    ...
    副作用:
        - ``add`` 与 ``add_one`` 在一个写事务中覆盖同名键；本轮训练/采样路径只以只读方式打开数据库。
    """
    ...
    def __getitem__(self, key):
        ...
        for i, key_this_db in enumerate(key_split):
            ...
            if len(key_list) == 2:
                ...
                # ``lmdb_name``：str，带分隔编号复合键片段中的子库名。
                # ``data_id``：str，带分隔编号复合键片段中的基础记录标识。
                # ``index_sep``：str，linking 等子库记录在基础标识后的分隔编号。
                lmdb_name, data_id, index_sep = key_list
                # ``fetch``：任意可 pickle 对象，使用 ``data_id/index_sep`` 作为当前子库记录键。
