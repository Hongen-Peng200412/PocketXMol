...
``NodeEncoder``、``RigidNet``、``TorsionNet`` 与可选 ``LocalPosUpdate`` 不在 reduced 配置的默认
构象/docking 路径中，其中旧类还依赖本文件未定义的 ``NodeBlock``，本轮只记录边界而不修复。
"""
...
class ContextNodeEdgeNet(Module):
    ...
    构造参数:
        ...
        - local_update: bool, True 时额外启用 ``LocalPosUpdate``；reduced 配置缺省为 False，本轮不展开该实验分支。
    ...
    def __init__(self, node_dim, edge_dim, hidden_dim,
        ...
        # ``kwargs.local_update``：bool|缺省，是否启用实验性局部位置更新。
        ...
        # ``self.local_update``：bool，是否在 L 层之后追加实验性局部三元组位置更新；reduced 配置为 False。
        self.local_update = kwargs.get('local_update', False)
        if self.local_update:
            # ``self.local_net``：LocalPosUpdate，实验分支的短程几何更新网络；默认不构造。
    ...
    def forward(self, h_node, pos_node, h_edge, edge_index,
        ...
        if self.local_update:
            # ``pos_node``：[N, 3] + [N, 3] -> [N, 3]；实验性短程三元组增量，默认配置不执行。
...
class TorsionNet(nn.Module):
    ...
    def forward(self, h_node, pos_node, force,
        ...
        """把节点位移投影为逐扭转键角度并旋转随动原子。

        输入参数:
            - h_node: FloatTensor，形状为 (N, D_n)，配体节点特征。
            - pos_node: FloatTensor，形状为 (N, 3)，旋转前配体坐标，单位 Å。
            - force: FloatTensor，形状为 (N, 3)，逐原子候选坐标增量，单位 Å。
            - h_edge: FloatTensor，形状为 (E, D_e)，与有向边逐列对齐的边特征。
            - edge_index: LongTensor，形状为 (2, E)，每列为有向边两端原子编号。
            - torsional_edge_anno: LongTensor，形状为 (2, R)，第一行是化学键编号，第二行是扭转有向边列号。
            - twisted_edge_anno: LongTensor，形状为 (2, T)，第一行引用扭转边行号，第二行是随动有向边列号。

        返回值:
            - pos_update: FloatTensor，形状为 (N, 3)，应用预测扭转角后的配体坐标，单位 Å。
            - angles: FloatTensor，形状为 (R, 1)，逐扭转键预测旋转角，单位 rad。
        """
        
        # ``i_bond_for_tor_edge``：LongTensor，形状为 (R,)，每条扭转边所属的批内化学键编号。
        # ``tor_edge``：LongTensor，形状为 (R,)，每条扭转边在 ``edge_index`` 中的有向边列号。
        i_bond_for_tor_edge, tor_edge =torsional_edge_anno
        # ``i_tor_edge_for_twisted_edge``：LongTensor，形状为 (T,)，每条随动边引用的扭转边行号。
        # ``twisted_edge``：LongTensor，形状为 (T,)，每条随动边在 ``edge_index`` 中的有向边列号。
        i_tor_edge_for_twisted_edge, twisted_edge = twisted_edge_anno
        
        # ``tor_left``：LongTensor，形状为 (R,)，逐扭转有向边的左端原子编号。
        # ``tor_right``：LongTensor，形状为 (R,)，逐扭转有向边的右端原子编号。
        ...
        # ``twisted_node``：LongTensor，形状为 (T,)，逐随动边的待旋转原子编号。
        # ``tor_end``：LongTensor，形状为 (T,)，逐随动边连接的扭转轴左端原子编号。
