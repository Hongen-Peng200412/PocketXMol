...
文件后半部分的 autoregressive 原子/键扩展函数不属于本轮构象与 docking 主链。
"""
...
def sample_loop3(batch, model, noiser, device=None, is_ar='', off_tqdm=False):
    ...
    输入参数:
        ...
        - device: torch.device|str|None，只透传给 linking 部件处理；本轮两任务不创建部件。
        - is_ar: str，只有以 ``ar`` 开头才进入额外自回归轮次；构象/docking 传 ``conf`` 或 ``dock``。
    ...
    cfd_traj = []
    # ``mol_parts``：list[Mapping]，linking AR 子图列表；构象/docking 主链为空。
    mol_parts = get_mol_parts_linking(batch, device=device)
    # ``data_mol``：PyG Batch|Mapping，当前写入 gt 轨迹的主图或 linking 部件。
    ...
    # ``step_ar``：额外自回归轮次编号；标准构象/docking 始终保持 0。
    step_ar = 0
    while True:
        ...
        if is_ar.startswith('ar'):
            if is_ar == 'ar':
                # ``batch``：AR 第一实现扩展/收缩第二部分后的新图状态，本轮两任务不进入。
                batch = noiser.outputs2batch_ar(batch, outputs, step_ar, cfd_traj)
            elif is_ar == 'ar2':
                # ``batch``：AR 第二实现更新后的图状态，本轮两任务不进入。
            ...
            if batch['node_p2'].shape[0] == 0:
                break
            # AR 尚有 ``node_p2`` 时累加轮次；标准路径不会修改该计数。
            step_ar += 1
        else:
            break
    ...
    try:
        ...
        for key in traj_dict.keys():
            # ``all_trajs[key]``：Tensor，变长实体后备路径的补齐结果；当前因上行异常不可达。
            all_trajs[key] = pad_and_stack(traj_dict[key], dim=0)
            # ``in_trajs[key]``：Tensor，补齐后的带噪输入轨迹。
            in_trajs[key] = pad_and_stack(in_dict[key], dim=0)
            # ``out_trajs[key]``：Tensor，补齐后的投影状态轨迹。
            out_trajs[key] = pad_and_stack(out_dict[key], dim=0)
            # ``raw_trajs[key]``：Tensor，补齐后的原始预测轨迹。
...
def pad_and_stack(tensor_list, dim):
    ...
    输入参数:
        - tensor_list: list[Tensor]，长度为 S，每项首维是可变实体数；其余维必须一致。
        - dim: int，补齐后 ``torch.stack`` 新建阶段轴的位置。

    返回值:
        - stacked: Tensor，一维输入叶形状为 (S, M) 或由 ``dim`` 置换后的等价形状，二维输入叶形状为 (S, M, F) 或等价形状，其中 M 为最大实体数。

    注意:
        - 一维类别张量以 -1 作为 padding 类别；二维特征/坐标张量在实体轴尾部补 0。
        - 正常构象/docking 路径使用 NumPy ``stack``；调用点在到达本后备逻辑前先抛 ``NotImplementedError``，因此当前不可达。
    """

    # ``tensor``：Tensor，``tensor_list`` 中当前读取第 0 维实体数的轨迹项。
    # ``size_list``：list[int]，长度为 S，第 s 项是 ``tensor_list[s]`` 第 0 维的实体数 M_s。
    size_list = [tensor.size(0) for tensor in tensor_list]
    # ``max_size``：int，统一 padding 的目标实体数。
    max_size = max(size_list)
    if tensor_list[0].dim() == 1:
        # ``tensor_list``：list[Tensor]，每项补成形状 (max_size,)，-1 表示无实体。
        # ``tensor``：Tensor，形状为 (M_s,)，当前待补齐的一维类别轨迹项。
        # ``size``：int，当前 ``tensor`` 的实体数 M_s。
        tensor_list = [F.pad(tensor, (0, max_size-size), mode='constant', value=-1)  # type = -1 means
                    for tensor, size in zip(tensor_list, size_list)]
    else:
        # ``tensor_list``：list[Tensor]，每项补成形状 (max_size, F)，实体尾部填 0。
        # ``tensor``：Tensor，形状为 (M_s, F)，当前待补齐的二维特征或坐标轨迹项。
        # ``size``：int，当前 ``tensor`` 的实体数 M_s。
...
def get_mol_parts_linking(batch, device=None):
    """提取 linking/autoregressive 任务的可选 gt 子图轨迹参照。

    构象与 docking Batch 不含 ``gt_*_p1/p2``，因此返回空列表。参数 ``device``
    在当前实现未使用，仅保留调用接口。
    """

    # ``parts``：list[dict]；每项包含一个子图的 gt 节点、坐标和半边张量副本。
    parts = []
    for i_part in [1, 2]:
        # if f'gt_node_type_p{i_part}' not in batch.keys:
        if f'gt_node_type_p{i_part}' not in batch:
            continue
        # ``part``：三个 gt 叶字段的克隆；不与原 Batch 共享可变存储。
        part = {
