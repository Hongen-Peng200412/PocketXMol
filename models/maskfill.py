...
class PMAsymDenoiser(Module):
    ...
    形状符号:
        ...
        - P: 批次中口袋原子总数。
        - E_p: 批次中口袋 kNN 有向边总数。
        - K_n: 原子类别数 ``num_node_types``。
        - K_e: 半边类别数 ``num_edge_types``。
        - D_n: 配体节点特征宽度。
        - D_e: 配体边特征宽度。
        - D_p: 编码后口袋节点特征宽度。

    构造参数:
        - config.pocket_dim: int, 编码后口袋节点宽度 D_p。
        - config.node_dim: int, 拼接原子 Embedding、两维 fixed prompt 和附加节点特征后的总宽度 D_n。
        - config.edge_dim: int, 拼接半边 Embedding 与两维 fixed prompt 后的总宽度 D_e。
        ...
        - num_node_types: int, 原子词表大小 K_n；当前包括配置元素类别和可选 mask 类。
        - num_edge_types: int, 半边词表大小 K_e；0 为非键、1..4 为真实键，末端可含 mask 类。
    ...
    前向输出字段:
        - pred_node: (N, K_n), 每个配体原子的干净类别 logits；未做 softmax。
        - pred_pos: (N, 3), 每个配体原子的去噪局部坐标，原点与 pos_in 相同，单位 Å。
        - pred_halfedge: (H, K_e), 每条无向半边的干净类别 logits；正反向隐藏特征先求和再解码。
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        super().__init__()
        # ``config.pocket_dim``：int D_p，编码后口袋节点宽度。
        # ``config.node_dim``：int D_n，拼接全部节点输入后的总宽度。
        # ``config.edge_dim``：int D_e，拼接全部半边输入后的总宽度。
        ...
        self.config = config
        # ``self.num_node_types``：int K_n，原子类别嵌入词表及分类头宽度。
        self.num_node_types = num_node_types
        # ``self.num_edge_types``：int K_e，半边类别嵌入词表及分类头宽度。
        ...
        # ``pocket_dim``：int，原始口袋 25 维离散特征经线性层后的节点宽度 D_p。
        pocket_dim = config.pocket_dim
        # ``self.pocket_embedder``：Linear；[P, D_p_raw] -> [P, D_p] 的口袋原始特征投影。
        ...
        # ``self.pocket_encoder``：Module，以 node_only=True 实例化；返回 (P, D_p) 且不更新口袋坐标。
        ...
        # ``node_dim``：int，fixed prompt 与可选附加通道拼接完成后的节点总宽度 D_n。
        node_dim = config.node_dim
        # ``edge_dim``：int D_e，半边 Embedding 与两维 fixed prompt 拼接后的总宽度。
        ...
        # ``self.nodetype_embedder``：Embedding，int64 (N,)->float (N, D_n-2-D_add) 的原子类别查表。
        self.nodetype_embedder = nn.Embedding(num_node_types, node_emb_dim)
        # ``self.edgetype_embedder``：半边类别 Embedding 宽度为 D_e-2；两维分别预留给 fixed_halfedge 与 fixed_halfdist。
        ...
        # ``self.denoiser``：Module，输入节点/边总宽度分别为 D_n/D_e，并用 D_p 口袋节点作为上下文。
        self.denoiser = denoiser_bb(node_dim, edge_dim,
                            context_dim=pocket_dim, **config.denoiser)

        # Output decoders
        # ``self.node_decoder``：MLP；[N, D_n] -> [N, K_n] 的原子类别 logits 解码头。
        self.node_decoder = MLP(node_dim, num_node_types, node_dim)
        # ``self.edge_decoder``：MLP；[H, D_e] -> [H, K_e] 的无向半边类别 logits 解码头。
        ...
        if 'confidence' in self.add_output:
            # ``self.node_cfd``：MLP；[N, D_n] -> [N, 1] 的原子类别置信度 logit 头。
            self.node_cfd = MLP(node_dim, 1, node_dim//2)
            # ``self.pos_cfd``：MLP；[N, D_n] -> [N, 1] 的坐标置信度 logit 头。
            self.pos_cfd = MLP(node_dim, 1, node_dim//2)
            # ``self.edge_cfd``：MLP；[H, D_e] -> [H, 1] 的无向半边置信度 logit 头。
    ...
    def forward(self, batch, **kwargs):
        ...
        输入字段:
            ...
            - batch.pocket_atom_feature: FloatTensor，形状为 (P, D_p_raw)，口袋元素、氨基酸和主链输入特征。
        ...
        pos_in = batch['pos_in']
        # ``h_node_in``：FloatTensor，形状为 (N, D_n-2-D_add)；按 ``node_in`` 的 K_n 类编号查表得到原子类别嵌入。
        h_node_in = self.nodetype_embedder(batch['node_in'])
        # ``h_halfedge_in``：FloatTensor，形状为 (H, D_e-2)；按 ``halfedge_in`` 的 K_e 类编号查表得到无向半边类别嵌入。
        ...
        # ``h_node_in``：[N, D_n-2-D_add] + [N, 2] -> [N, D_n-D_add]；只拼接节点特征维。
        h_node_in = torch.cat([h_node_in, node_extra], dim=-1)
        # ``h_halfedge_in``：[H, D_e-2] + [H, 2] -> [H, D_e]；只拼接半边特征维。
        h_halfedge_in = torch.cat([h_halfedge_in, halfedge_extra], dim=-1)

        # ``n_halfedges``：int，无向完全图半边总数 H；用于把去噪后的双向边重新配对。
        ...
        # ``edge_index``：[2, H] + [2, H] -> [2, 2H]；后半通过交换两行生成反向边，每列在 GNN 中解释为目标、来源端点。
        edge_index = torch.cat([halfedge_index, halfedge_index.flip(0)], dim=1)
        # ``h_edge_in``：[H, D_e] -> [2H, D_e]；正反向边初始特征相同，按前 H 行与后 H 行一一配对。
        h_edge_in = torch.cat([h_halfedge_in, h_halfedge_in], dim=0)
        # ``edge_extra``：[H, 2] -> [2H, 2]；正反向边共享同一 fixed edge/distance prompt。
        ...
        if 'is_peptide' in self.addition_node_features:
            ...
            # ``h_node_in``：[N, D_n-1] + [N, 1] -> [N, D_n]；完成配置要求的节点总宽度。
            h_node_in = torch.cat([h_node_in, is_peptide], dim=-1)
        
        # ``h_pocket``：FloatTensor，形状为 (P, D_p)；把口袋离散输入通道线性投影到口袋编码宽度。
        h_pocket = self.pocket_embedder(batch['pocket_atom_feature'])
        # ``h_pocket``：FloatTensor，形状为 (P, D_p)；只沿固定 ``pocket_knn_edge_index`` 更新口袋节点特征，不改变 ``pocket_pos``。
        ...
        # ``h_node``：FloatTensor，形状为 (N, D_n)，完成 L 个联合 block 后的配体节点隐藏特征。
        # ``pos_node``：FloatTensor，形状为 (N, 3)，完成 L 个坐标增量后的配体局部坐标，单位 Å。
        # ``h_edge``：FloatTensor，形状为 (2H, D_e)，与双向 ``edge_index`` 列对齐的最终边隐藏特征。
        ...
        # ``pred_node``：FloatTensor，形状为 (N, K_n)；逐原子干净类别 logits，未归一化。
        pred_node = self.node_decoder(h_node)
        # ``pred_halfedge``：[2H, D_e] -> [H, D_e] -> [H, K_e]；一一配对的正反向隐藏特征求和而不取平均，再解码无向半边 logits。
        ...
        # ``return.pred_node``：FloatTensor，形状为 (N, K_n)，原子类别 logits。
        # ``return.pred_pos``：FloatTensor，形状为 (N, 3)，去噪局部坐标，单位 Å。
        # ``return.pred_halfedge``：FloatTensor，形状为 (H, K_e)，半边类别 logits。
        ...
        return {
            ...
            'pred_pos': pred_pos,
            # ``return.pred_halfedge``：FloatTensor，形状为 (H, K_e)；无向半边类别 logits。
