...
密度模型沿用同一训练、采样和评价入口，通过model.density接入56通道与单次U-Net；D1/D4已有性能预实验，D2/D3已实现，四类结构的中心/包络真实入口均已在前置集成验收；本轮只运行D1、D2，D3/D4不再执行。密度模型额外需要einops 0.8.1，见[requirements-density.txt](../requirements-density.txt)；无密度配置不加载该依赖。预实验曾仅在独立deps提供此依赖；2026-09-14集成时已安装到PocketXMol专用venv，公共Conda未改动。依赖安装属于环境准备命令，不能与正式训练命令混记。
...
一张卡仅运行一个模型；完成本轮分配的全部训练、best测试和CPU评价后回到try_lock等待，保留after_lock。本轮无密度和D1起始72×1，D2起始36×2；OOM按72→36→24降批、累积相应1→2→3，配置global_batch_size保持72。接受原reduce_batch偶发裁批和累积梯度丢失，不增加补样或梯度补偿。仍无法运行则报告实际问题。默认不再开展I/O改善或参数搜索，也不因D1预实验利用率未达目标自动优化。运行中观察实际I/O和GPU；只有主动发现I/O较此前可比训练变慢时才定位实际瓶颈并改善，保持实例抽样和科学定义。W&B始终online，不能自动切换offline。
...
每张GPU完成本轮当前模型的best规定测试、CPU评价与记录后，保留after_lock并回到try_lock，不自动启动其他模型。评价可用该GPU作业自带CPU；现有配置8个评价进程。训练保留原val/loss、调度及best，训练后不提交完整验证集采样或评价。旧validation配置与候选仅作历史，不继续执行；本次修正编码复验使用独立产物目录，已有结果全部保留。
...
本次官方冻结权重复验使用 `sample-official-SMILES-test.yml`，产物为`/storage/penghongen/PocketXMol/sampling/official-SMILES/test`。在获准GPU内按以下短命令生成完整测试候选；所有协议采样完成后，可在同一作业内运行评价：
...
前者核对RA、bf16原loss、36×2=72、官方参数加载与已停止检查点恢复。后者覆盖中心T0-RA与包络T0-RA，各运行2次更新、1个验证批，再对首条validation生成2个3步候选并评价；不设姿态质量阈值、不读test划分，global batch仍为72，OOM明确失败。两者关闭W&B，产物仅在独立pytest临时目录；只在当前获准资源内按实际改动需要执行，不把接口示例当作新增实验安排。
