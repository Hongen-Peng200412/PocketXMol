...
旧配体编码的六个无密度模型及其官方对照已经收口；修正SMILES芳香键编码后的RA＋T0无密度复验和官方冻结权重测评也已完成。第一次密度实验只保留已经实际执行的D1、D2事实，D2包络继续完成既有训练、best E测试和CPU评价；D3、D4不启动。当前正式任务是[9-15方案](../想法/方案草稿/9-15.md)定义的`local_cov`中心与包络两个模型。下列命令说明稳定接口，实际release、资源和进度由`日志/第三次正式实验/local_cov/`中的三份日志记录。
...
`local_cov`通过`model.density.name=local_cov`接入80³固定106通道网格，并在六个去噪块中按当前配体坐标读取11³局部块。中心与包络正式配置分别为`local_cov-C-T0-RA.yml`和`local_cov-E-T0-RA.yml`，首发批量为36、累积2次、配置全局批量72；局部卷积分块首选4096，显存不足时先降到2048，再按批准规则改为24×3。该结构不依赖第一次密度实验使用的U-Net、全局注意力、体素选择或辅助分割目标。
