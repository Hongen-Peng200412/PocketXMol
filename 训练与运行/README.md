...
旧配体编码的无密度运行已完成，六模型实际命令和配置来源保存在[非密度总日志](../日志/第一类实验（不加密度信息）/总日志&分析/总日志.md)及逐模型日志。用户已批准两项预实验及其后的D1→D4→D2→D3八个RA＋T0模型；前置集成与验收、既定审查、双线收口完成后先汇报并停止推进；正式密度训练与无密度复验等待用户再次明确允许。用户已批准官方SMILES芳香编码、bf16-mixed训练和FP32张量推理，以及378693专用的两个无密度RA＋T0重训与官方C0/C5/E测试；371591和379402继续密度中心／包络职责。当前集成验收状态以[预实验总日志](../日志/预实验（一 --二之间）/总日志.md)为准。下列命令说明稳定接口，实际执行范围、获准资源与运行状态由当前契约和所属实验日志确定。
