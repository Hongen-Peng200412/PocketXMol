...
2026-09-14最新授权：前置集成已经收口，用户接受官方输入与T0机制实质对齐，包括已披露的GPU数值重复性与AdamW检查局限，不重跑整套预实验。现在端到端完成获准正式训练、测试、CPU评价与收口。371591／gnode09依次完成D1中心、D1包络、D4中心、D4包络；378693／gnode10依次完成D2中心、D2包络、D3中心、D3包络；379402_0（实际JobId379402）／gnode09依次完成无密度中心RA＋T0、包络RA＋T0复验及官方冻结权重C0/C5/E测试。每个模型训练、best测试和本资源CPU评价完成后才进入下一模型，三卡可并行。当前状态见[密度总日志](../日志/第二类实验（密度分支的训练）/总日志.md)与[无密度复验总日志](../日志/RA-T0第二次严格消融/总日志.md)。
