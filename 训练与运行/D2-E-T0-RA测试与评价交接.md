...
**准备状态（2026-09-17）：只读核对服务器入口，命令和配置模板仅落盘于本文件；没有部署、改锁或启动。** 09:54 核对时，实际动态命令只有训练，尚未串接推理；原冻结 release 没有 D2 包络测试配置。最终 best 尚未确定，下面的配置模板必须在训练正常结束后填写真实 checkpoint 路径，不能原样启动。

## 交接与运行条件

接手前由用户明确移交后续操作责任，并让原负责 agent 停止对该作业的后续提交和控制。此文件的创建不表示移交通知已经送达，也不表示原 agent 已停止操作。

接手 agent 在推进其另行获准工作时，每约 30 分钟检查一次 D2 包络。训练继续使用原参数与在线 W&B，不重启、不更换权重、不修改正在执行的动态命令。确认训练结束后只启动尚未完成的阶段。
