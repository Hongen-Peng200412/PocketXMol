...
def test_cached_density_matches_reencoding_in_real_sampling(prepared_data, monkeypatch, mode):
    """逐步比较同一输入的缓存/重新编码输出, 并核对跨候选批次只编码一次."""
    torch.manual_seed(83)
    training, dataset, featurizer, config, _ = sampling_context(prepared_data, 'RA', 'validation')
    training.model.density = dict(mode=mode, attention_backend='sdpa', distance_bias=True, checkpoint=False)
