...
本目录提供精确 prepared SMILES 的公共图读取、实例坐标读取、口袋构造及原 PocketXMol free docking 接口。`pdb_id＋candidate_id` 定位沉积实例，`prepared_smiles` 定义化学图身份。SMILES公共芳香键编码与历史CCD单双键编码不是相同模型输入；迁移是否满足旧模型数值契约，须以[SMILES预实验的真实验收](../日志/预实验（一 --二之间）/1-SMILES构图与GPU验收.md)为准，不能只凭RDKit化学同构判断。

推荐先阅读`smiles.py`中的公共图和坐标接口，再读`dataset.py`的口袋装配，最后读`sampling.py`与`evaluation.py`。`assets.py`只负责标准受体和完整残基口袋选择；`preparation.py`检查现成图、语言和标签，不再构建ligand_object图或重新枚举自同构。

## 产物位置

下列SMILES公共包与候选迁移坐标、清单已在服务器核对存在。候选迁移完成不表示模型数值验收通过。配置中的`smiles_root`、`smiles_coords_root`、`manifest_root`明确选择对应目录。
...
新建数据准备仍由`prepare`组织`index、objects、samples、freeze`。`objects`现在只检查已有公共图和排列；`samples`核对SMILES顺序坐标、现有语言向量与标签。语言身份核对pdb_id、candidate_id、精确prepared_smiles、已有model_smiles以及`model_name="SMI-TED Light 289M"`，不再读取object_key匹配。

`preparation/sources.jsonl`含split/pdb_id/candidate_id/prepared_smiles；`objects_{id}.jsonl`含prepared_smiles/status/reason/atom_count/canonical_smiles，成功如`{"prepared_smiles":"CCO","status":"ok","reason":"","atom_count":3,"canonical_smiles":"CCO"}`；失败如`{"prepared_smiles":"invalid","status":"excluded","reason":"smiles_graph: invalid","atom_count":null,"canonical_smiles":null}`。`canonical_smiles`只核对语言输入，不替代精确身份。

`samples_{id}.jsonl`为通过检查并附n_heavy_atoms的实例，`excluded_{id}.jsonl`与`index_excluded.jsonl`保留具体身份和首个reason；`summary.json`汇总四划分、身份数、视图数和排除计数。文件存在不代表验收通过。此次直接消费迁移后的冻结清单，不执行重新准备、重新筛选或重新冻结。
...
正式准备入口为 `scripts/prepare_docking.py`，按 `index → objects → samples → freeze` 执行。每 CPU 任务 8 核、最多 12 份并发；正式 Slurm 命令见 `训练与运行/README.md`。`tests/test_docking_data.py` 使用构造资产检查筛选、原子编号、几何与视图；SMILES迁移与验收记录见[预实验日志](../日志/预实验（一 --二之间）/1-SMILES构图与GPU验收.md)。本次使用迁移清单，不重新执行freeze。
