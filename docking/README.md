...
下列结构来自当前代码，尚未完成服务器正式准备。默认路径由 `configs/docking/prepare.yml` 指定。两个目录树中的 `data` 是同一个物理目录，分别列出稳定接口与准备诊断。
...
偏移方向在球面均匀，半径独立服从 `Uniform(0,5)`，不是在球体积内均匀采样。训练每次取样重新生成；有限验证与推理读取冻结值。以下只展示格式，不代表正式通过状态或实际冻结种子：
