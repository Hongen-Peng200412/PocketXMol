...
正式输出由 manifest_root 和 derived_root 决定; 中间诊断保存在 manifest_root/preparation, 字段见 prepare. 公共SMILES图与已迁移坐标只读, 不复制密度图, 不修复例外、不重算语言表征.
"""
...
from docking.smiles import read_smiles_graph, read_smiles_coords
...
def read_occurrences(root, split, pdb_id, candidate_ids):
    ...
    返回值:
        - records: list[dict], 每条含 split、pdb_id、candidate_id、prepared_smiles; candidate_id 即 occurrence_id, prepared_smiles 如 CCO.
        - excluded: list[dict], 相同身份字段加 reason; reason 是具体不满足的条件或读取异常, 未知 prepared_smiles 为空字符串.
    """
    records, excluded = [], []
    try:
        occurrences = read_jsonl(root / "parse" / pdb_id / "occurrences.jsonl")
        prepared = {int(item["candidate_id"]): item["smiles"] for item in read_jsonl(root / "stage1_preparation_box_pool_2/ligand_language_models/prepared" / pdb_id / "prepared_smiles.jsonl")}
    except (OSError, ValueError) as error:
        ids = [-1] if candidate_ids is None else sorted(candidate_ids)
        return [], [dict(split=split, pdb_id=pdb_id, candidate_id=cid, prepared_smiles="", reason=f"occurrence_metadata: {error}") for cid in ids]
    found = set()
    for occurrence in occurrences:
        ...
        record = dict(split=split, pdb_id=pdb_id, candidate_id=candidate_id, prepared_smiles=prepared.get(candidate_id))
        ...
        elif not record["prepared_smiles"]:
            reason = "missing_prepared_smiles"
        else:
            records.append(record)
            continue
        excluded.append(dict(record, reason=reason))
    if candidate_ids is not None:
        excluded.extend(dict(split=split, pdb_id=pdb_id, candidate_id=cid, prepared_smiles="", reason="occurrence_metadata_missing_id") for cid in sorted(candidate_ids - found))
    return records, excluded


def prepare_object(smiles_root, prepared_smiles):
    """核对公共 SMILES 图和排列是否可供原 dock 模型读取, 不复制或重新生成图资产.

    smiles_root 为公共包目录, prepared_smiles 为精确身份字符串. 返回 prepared_smiles、status、reason、atom_count 和 canonical_smiles; 成功 status=ok、reason 为空, 失败状态 excluded 并保留原因, 原子数和规范字符串为 None. canonical_smiles 只核对既有语言模型输入, 不替代精确身份.
    """
    record = dict(prepared_smiles=prepared_smiles, status="excluded", reason="", atom_count=None, canonical_smiles=None)
    try:
        graph = read_smiles_graph(smiles_root, prepared_smiles)
        record.update(status="ok", atom_count=len(graph["element"]), canonical_smiles=Chem.MolToSmiles(graph["mol"], isomericSmiles=False))
    except (OSError, ValueError, KeyError, RuntimeError) as error:
        record["reason"] = f"smiles_graph: {error}"
    return record


def prepare_pdb(root, derived_root, language_root, smiles_root, smiles_coords_root, records, graph_results):
    ...
    输入参数:
        ...
        - records: list[dict], 同一 PDB 的候选记录, 字段为 split、pdb_id、candidate_id、prepared_smiles.
        - smiles_root: Path, 公共图和自同构包目录.
        - smiles_coords_root: Path, 已迁移为公共原子顺序的逐 PDB 坐标包目录.
        - graph_results: dict[str, dict], 精确 prepared_smiles 到 prepare_object 返回记录, 失败图排除其相关实例.
    ...
    eligible = []
    for record in records:
        result = graph_results[record["prepared_smiles"]]
        if result["status"] == "ok":
    ...
    with area_archive:
        for record in eligible:
            ...
            try:
                graph = read_smiles_graph(smiles_root, record["prepared_smiles"])
                # (N, 3), 迁移包只含完整沉积重原子, 原子顺序与公共图一致.
                coords = read_smiles_coords(smiles_coords_root, pdb_id, candidate_id, record["prepared_smiles"])
                if coords.shape != (len(graph["element"]), 3) or not np.isfinite(coords).all():
                ...
                canonical_smiles = graph_results[record["prepared_smiles"]]["canonical_smiles"]
                ...
                with np.load(language_root / pdb_id / f"candidate_{candidate_id}.npz", allow_pickle=False) as embedding:
                    ...
                    if language["pdb_id"] != pdb_id or str(embedding["pdb_id"].item()) != pdb_id or int(embedding["candidate_id"].item()) != candidate_id or str(embedding["prepared_smiles"].item()) != record["prepared_smiles"] or str(embedding["model_name"].item()) != "SMI-TED Light 289M":
            ...
            kept.append(dict(record, n_heavy_atoms=len(graph["element"])))
    return kept, excluded
...
def prepare(config, stage, shard_id, shard_count):
    ...
    输入参数:
        ...
        - config.smiles_root: str, 精确 SMILES 公共图及对称排列目录.
        - config.smiles_coords_root: str, 已迁移的逐 PDB 重原子坐标包目录.
        - config.derived_root: str, 只保存 ligand_area 的本项目派生根路径.
    ...
    清单和诊断文件:
        - preparation/sources.jsonl: 候选身份记录, 含 split、pdb_id、candidate_id、prepared_smiles, 不含图或坐标.
        - preparation/index_excluded.jsonl: 身份条件排除记录, 上述字段加 reason.
        - preparation/objects_{id}.jsonl: 当前分片每个模板的 prepared_smiles、status、reason、atom_count、canonical_smiles, 字段见 prepare_object.
    ...
    index/freeze 不自行申请其它资源. objects 按完整 prepared_smiles 排序切片, samples 按完整 PDB 排序切片, 因而同一派生文件没有跨 array 写入竞争.
    """
    ...
    if stage == "index":
        ...
        objects = sorted({record["prepared_smiles"] for record in sources})[shard_id::shard_count]
        results = Parallel(n_jobs=config.workers)(delayed(prepare_object)(Path(config.smiles_root), prepared_smiles) for prepared_smiles in objects)
        ...
        graph_results = {record["prepared_smiles"]: record for index in range(shard_count) for record in read_jsonl(work_root / f"objects_{index}.jsonl")}
        ...
        results = Parallel(n_jobs=config.workers)(delayed(prepare_pdb)(root, derived_root, Path(config.language_root), Path(config.smiles_root), Path(config.smiles_coords_root), by_pdb[pdb_id], {record["prepared_smiles"]: graph_results[record["prepared_smiles"]] for record in by_pdb[pdb_id]}) for pdb_id in pdb_ids)
        ...
        for index, record in enumerate(kept):
            ...
            # (3,), 方向均匀、半径均匀; 仅生成评价定位条件, 训练不抽取中心偏移.
        ...
        for record in kept:
            if record["split"] == "test":
                by_object[record["prepared_smiles"]].append(record)
        for prepared_smiles in sorted(by_object):
            records = by_object[prepared_smiles]
        ...
        for split in ("train", "validation", "calibration", "test"):
            ...
            summary["splits"][split] = dict(occurrences=len(records), pdbs=len({record["pdb_id"] for record in records}), objects=len({record["prepared_smiles"] for record in records}), views=dict(Counter(view for record in records for view in record["views"])))
