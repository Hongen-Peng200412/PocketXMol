...
**A800上的正式训练已完成，E完整测试正在推理。** 截至15:01，已完成175／446实例，8750个候选全部成功；最终姿态指标须待完整推理及CPU评价。训练在30400次更新后因第三次学习率下降停止，实际best为21600步、原E val/loss=1.6228482723236084；它是全部38个定期检查点的最低原验证损失，1236个模型参数键均有限。全部检查点、last及训练W&B记录保留。
...
| 测试与评价产物 | `/storage/penghongen/PocketXMol/sampling/B-E-T0-RB/test/`；run.json及175个实例的完整候选已产生，评价尚未开始 |
| 评价 W&B | 尚未创建 |
...
当前采样主进程33421仍在job_378693运行；after_lock存在，try_lock与kill_lock不存在。已完成175个实例、8750个候选，失败0；全部已有result.json均为complete，batch50、50候选和100步不变。已完成实例的采样耗时累计5147.28秒，此数不是整个作业墙钟时间。未见Traceback、OOM、非有限损失或降低batch的记录；姿态质量尚待完整CPU评价。
