...
**A800上的正式训练中，测试尚未开始。** 378693／gnode10 的本次日志已到22982次更新，学习率2e-5；最近完成22400步验证，val/loss=1.72378，日志当前最低值为21600步的1.62285。它只是训练中观测，尚不是最终测试best。未见Traceback、OOM或非有限损失记录。
...
本次只读检查来自 `/home/penghongen/Feedback/Pocket_Plus/allocations/378693/out` 和 `err`，从启动记录指定的25869442／59362字节读取；after_lock存在、try_lock不存在。训练摘要尚未生成。继续原E训练及val/loss，停止后核实实际best，再进行446实例的E完整测试和同作业CPU评价。
