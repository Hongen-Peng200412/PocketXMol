...
验收通过后执行的短入口为 `bash 训练与运行/sh/train_docking.sh B-E-T0-RA-SMILES`，当前未执行。训练结束读取实际best后分别记录采样和评价命令，不生成完整验证集候选。
...
**有效重训、完整E测试和评价均已完成。** ALL 的 Top-1／Top-5／oracle 成功率为65.47%／80.94%／90.36%；446实例、22300候选全部生成并评价成功。有效训练在26400次更新后停止。
...
| 项目 | 当前事实 |
|---|---|
| 训练产物根 | `/storage/penghongen/PocketXMol/training/B-E-T0-RA-nonemptyE/` |
| 正式测试检查点 | `checkpoints/step=21600.ckpt`；E val/loss=1.6292482614517212 |
| 训练 W&B | [lukfzmf3](https://wandb.ai/pencounkdual-111/PocketXmol_raw/runs/lukfzmf3) |
| 测试与评价产物 | `/storage/penghongen/PocketXMol/sampling/B-E-T0-RA-nonemptyE/test/` |
| 评价 W&B | [ajpum4py](https://wandb.ai/pencounkdual-111/PocketXmol_raw/runs/ajpum4py) |
...
配置为 `configs/docking/B-E-T0-RA.yml`，科学参数和共同资产保持不变。有效重训使用ec06dbd基线加c9cc4a1空E修复。仅E训练与val/loss在组批前跳过空口袋：46个训练身份、0个验证身份，有效E训练65244；正式测试失败仍保留分母，冻结清单不变。完整审计及首次异常证据见本文末尾。
...
首次异常训练使用ec06dbd运行副本 `/home/penghongen/Feedback/PocketXMol/releases/PocketXMol_fa0d957b2d3f/PocketXMol`。当时的正式训练在该冻结release中执行：
...
包络T0-RA代码和72×1配置已经通过两遍主代理自查、两轮全面独立审查及真实非测试样本GPU验收，见 [共同准备](../实现与共同数据准备.md) 和 [T0纠偏](1-B-C-T0-RA.md)。本次沿用通过验收的配置，不重复试训或学习率搜索。启动前再次核对现有资源已回到try_lock、旧评价进程退出、after_lock保留、新训练根不存在及release配置与本地一致。
