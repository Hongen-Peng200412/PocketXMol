...
用户注释单独保存为6cd023f；两处生产修复、测试、配置说明和当前契约保存为ec01242。使用原sync_code.ps1非删除同步，保留服务器Git和旧release。本轮CPU／GPU检查共用冻结源码 `/home/penghongen/Feedback/PocketXMol/releases/PocketXMol_5a4259c2b381/PocketXMol`。
...
真实源检查直接读取已冻结train的5ftl/0，不重新准备任何科学资产。GPU资源检查使用现有real_data测试的B-C-T0-RA项，从官方参数做2次更新、原C0监督验证和C5采样评价；它是必要短检查，不产生正式模型成绩。服务器命令、release、launch及通过证据在实际执行后补充。
...
本次纠偏安排两轮独立审查，分别负责Git与函数布局、注释与文档、科学逻辑与验收覆盖；审查只覆盖本次中心契约修复及其实际调用链。此前完整实现已完成的审查不重新扩大范围。结果在收到后记录；正式重训仍等待本次审查及服务器必要验收通过。
...
改后主代理核对两个受改测试函数的职责、调用与关键行含义，再执行相同本地测试入口，basetemp=tmp/pxm-t0-correction-20260910/pytest-boundary-4，选择 `tests/test_docking_centers.py -k training_boundary`；2项通过、12项未选，耗时17.57秒。第二轮三类独立审查在此基础上进行，本轮结束后只对实际发现的问题窄核。
...
新产物根为 `/storage/penghongen/PocketXMol/training/B-C-T0-RA-C0/`；使用原入口已有的--logdir参数，不新增重训专用执行器。不给--resume，明确从规定官方pxm模型参数初始化并重新建立优化器和调度状态，创建独立W&B run。实际run id、版本和启动状态待成功启动后记录，不能承接旧错误T0参数。

## 计划与实现差异

有害差异已定位并获用户明确纠正：旧中心T0训练和监督验证错误地使用C5。新代码已改为C0，T1仍保留配套的C5定位与平移机制。共同科学资产无需变化；两遍主代理自查已完成，当前待完成服务器必要验收与两轮独立审查、正确T0重训，以及原目标中的其余模型、完整验证／测试、报告和双线Git收口。
