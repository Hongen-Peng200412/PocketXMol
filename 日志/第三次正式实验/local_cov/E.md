...
2026-09-20 23:20状态快照：正式训练已在第26400次优化器更新后正常结束，最终模型选择best为`step=24800.ckpt`、`val/loss=1.4366419315338135`。379402已使用该best进入完整测试集E推理；配置固定batch50、每实例50候选和100步，继续使用不可变release `ef9dd39`。启动时标准输出已进入采样入口，尚未产生首个实例记录；`after_lock`保留，CPU评价将在采样成功后由同一动态命令执行。
