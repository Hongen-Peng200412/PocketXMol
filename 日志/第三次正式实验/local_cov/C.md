...
2026-09-20 07:06状态快照：正式训练在第26400次优化器更新后正常结束，最终模型选择best为`step=24800.ckpt`、`val/loss=1.61287522315979`。best的C0/C5完整测试正在378693／gnode10运行；C0已完成87/446个实例、4350/4350个候选，全部成功，C5尚未开始，采样错误输出为空。采样成功后将由同一动态命令执行CPU评价；`after_lock`和全部训练产物保留。
