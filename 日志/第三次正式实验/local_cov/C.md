...
2026-09-20 15:32状态快照：正式训练在第26400次优化器更新后正常结束，最终模型选择best为`step=24800.ckpt`、`val/loss=1.61287522315979`。best的C0/C5完整测试正在378693／gnode10运行；C0已完成446/446个实例、22300个候选，C5已完成414/446个实例、20700个候选，累计43000个候选全部成功，采样错误输出为空。采样成功后将由同一动态命令执行CPU评价；`after_lock`和全部训练产物保留。
