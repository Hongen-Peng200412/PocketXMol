...
2026-09-14 17:21状态：最后一轮批量读出8组合门控通过，A800密度测试11 passed；实际medium精度下所有活跃参数梯度均对照highest的完整FP32数学参考。D4第9次完整训练清单完成16次真实AdamW更新，热态4.697秒、GPU97.10%、等待0.018%、显存44.36GB。D1第8次24×3/32 worker完成24次更新，但复用第6次种子2023样本前缀，操作系统文件缓存与并发收益混合，不能仅归因于32 worker。第10–14次正在比较D1的24/36/72和D4的36/72；距离开关完整更新比较在其后进行。
