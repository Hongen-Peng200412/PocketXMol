...
当前状态（2026-09-15 13:42／13:44）：B-C-T0-RA-SMILES：训练完成，best21600；C0完成446/446实例、22300候选成功；C5完成295/446实例、14750候选全部成功，尚无CPU指标。
...
| best／W&B／指标 | B-C-T0-RA-SMILES：训练完成，best21600；C0完成446/446实例、22300候选成功；C5完成295/446实例、14750候选全部成功，尚无CPU指标 |
...
启动核查（07:41）：`test/run.json`确认本次best21600、精确SMILES资产、C0/C5、50候选、100步、batch50；strict权重加载已经通过。首个C0实例11jb／0生成50/50候选，耗时35.64秒；stderr未见报错。GPU显存3079 MiB、利用率98%为07:40单点快照。采样不另建W&B记录，沿既有实现由后续CPU评价将指标在线记录，训练run维持finished。进入60分钟分段等待，不改采样预算或I/O策略。
...
核查（2026-09-15，gnode09 08:42／gnode10 08:44）：训练完成，best21600／1.7227385；C0已完成122/446实例，6100/6100候选生成成功，C5尚未开始，尚无CPU汇总指标。D1与D2累计OOM为0，损失有限，吞吐无明显下降；不优化I/O。无密度测试保持batch50、50候选、100步，尚无失败实例。继续60分钟分段等待。

核查（2026-09-15，gnode09 09:44／gnode10 09:46）：训练完成，best21600／1.7227385；C0已完成264/446实例，13200/13200候选生成成功，C5尚未开始，尚无CPU汇总指标。未见新异常退出，训练损失有限，吞吐无明显下降；不优化I/O。无密度测试保持batch50、50候选、100步，尚无失败实例。继续60分钟分段等待。

核查（2026-09-15，gnode09 10:45／gnode10 10:47）：训练完成，best21600／1.7227385；C0已完成383/446实例，19150/19150候选生成成功，C5尚未开始，尚无CPU汇总指标。D1与D2累计OOM为0，损失有限，吞吐无明显下降；不优化I/O。无密度测试保持batch50、50候选、100步，尚无失败实例。继续60分钟分段等待。

资源调整期间核查（2026-09-15 11:33／11:35）：训练完成，best21600；C0完成446/446实例、22300/22300候选生成成功；C5完成23/446实例、1150/1150候选成功，尚无CPU评价指标。其余既有进程未受接管影响，未改科学配置。

核查（2026-09-15 12:40／12:42）：B-C-T0-RA-SMILES：训练完成，best21600；C0完成446/446实例、22300候选成功；C5完成156/446实例、7800候选全部成功，尚无CPU指标。三个训练run远端running且持续接收指标，累计OOM均0；无密度训练run finished、测试仍在正常生成。吞吐未见明显下降，不调整I/O或科学配置，继续60分钟分段等待。

核查（2026-09-15 13:42／13:44）：B-C-T0-RA-SMILES：训练完成，best21600；C0完成446/446实例、22300候选成功；C5完成295/446实例、14750候选全部成功，尚无CPU指标。三个训练run远端running且持续接收指标，累计OOM均0；无密度训练run finished、测试仍在正常生成。吞吐未见明显下降，不调整I/O或科学配置，继续60分钟分段等待。
